                ...
                <div class="max-w-md mx-auto">
                    ...
                    <div class="divide-y divide-gray-200">
                        <div class="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
                            ...
                            <div class="relative">
                                <button @click="login(); $event.preventDefault()" class="bg-blue-500 text-white rounded-md px-2 py-1">Submit</button>
                            </div>
                            <Notification :status="status" :object="''" :action="'Đăng nhập'" :isShowNoti="showNoti"
                                v-if="showNoti == 'Ok'">
                            </Notification>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        async login() {
            await this.$axios.post("http://localhost:8080/api/v1/auth/authenticate", this.authen).then((response) => {
